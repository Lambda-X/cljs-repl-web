...
(defn highlight-prompt!
  "Given a jquery selector, applies highlighting on it. Returns the html
  with highlighted code."
  ([jq-selector]
   (highlight-prompt! jq-selector "clojure"))
  ([jq-selector language]
   ;; (.-outerHTML )
   (let [non-empty (filter (complement #(empty? (element-text %)))
                           ($ jq-selector))]
     (println (count non-empty))
     (if (seq non-empty)
       (let [non-empty-el (first non-empty)
             hlighted-str (.-value (js/hljs.highlight language (element-text non-empty-el) true))
             hlighted-el (.addClass (js/$ hlighted-str) "jqconsole-prompt-text")]
         (println (string? hlighted-str))
         (.html ($ non-empty-el) hlighted-str))
